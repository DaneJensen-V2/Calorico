...
class NutritionCounter: UIView {
    ...
    var type: NutritionTypes = .calorie
    ...
    public var serving: Double = 1.0 {
        didSet {
            servingSet()
        }
    }

    public var trueValue: Int = 0 {

     didSet {
        print("Setting \(self.type) value to \(trueValue)")
    ...
    public var totalValue: Int = 100 {
        didSet {
            mySlider.maximumValue = Float(totalValue)
        }
    }

    public var displayValue: Int = 0

    init(totalValue: Int, currentValue: Int, type: NutritionTypes) {
    ...
    func updateUI(animated: Bool) {
        if self.type == .calorie {
            self.textField.text = String(displayValue)
            self.mySlider.setValue(Float(displayValue), animated: animated)

        } else {
            self.textField.text = String(displayValue) + "g"
            self.mySlider.setValue(Float(displayValue), animated: animated)
    ...
    func addProgressBar(icon: UIView) {
        ...
        next.tintColor = colors.darkBlueBG
    ...
    @objc func nextClicked() {
        print("Next")
        NotificationCenter.default.post(name: Notification.Name("nextClicked"), object: type)

    }

    @objc func textFieldDidEnd(_ textField: UITextField) {
        if type == .calorie {
            trueValue = Int(textField.text ?? "0") ?? 0
    ...
    func customizeSlider(counterType: NutritionTypes) {
       switch counterType {
